...
export async function generateMetadata({ params }) {
  ...
  try {
    const { data: artist } = await fetchAPI(`/artists/${id}`, {
      access_token
    });
...
export default async function page({ params }) {
  ...
  try {
    ...
    const [topTracks, albums, appearsOn, relatedArtists, isFollowed] =
      await Promise.all([
        // Get top tracks
        fetchAPI(`/artists/${id}/top-tracks`, {
          access_token
        }).then(({ data }) => data),

        // Get albums
        fetchAPI(`/artists/${id}/albums`, {
          access_token,
        ...
        // Get related artists
        fetchAPI(`/artists/${id}/related-artists`, {
          access_token
        }).then(({ data }) => data),

        // Check if artist is followed
        fetchAPI(`/me/following/contains`, {
          access_token,
