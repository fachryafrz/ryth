...
export async function generateMetadata({ params }) {
  ...
  try {
    const { data: track } = await fetchAPI(`/tracks/${id}`);
  ...
  try {
    ...
    const [artistsDetails, artistsTopTracks, albums, relatedArtists, isSaved] =
      await Promise.all([
        ...
        Promise.all(
          track.artists.map(async ({ id }) => {
            const { data } = await fetchAPI(`/artists/${id}/top-tracks`);
            return data;
          }),
        ),

        // Get albums
        fetchAPI(`/artists/${primaryArtist.id}/albums`).then(({ data }) => data),

        // Get related artists
        fetchAPI(`/artists/${primaryArtist.id}/related-artists`).then(
          ({ data }) => data,
        ),

        // Check if track is saved
        fetchAPI(`/me/tracks/contains`, {
          params: { ids: id },
        })
          .then(({ data }) => data[0])
          .catch(() => false),
